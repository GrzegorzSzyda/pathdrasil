...
export class ConversationService {
  ...
  private prompt(
    ...
    return `Jesteś konsultantem technicznym Pathdrasil. Możesz przeglądać repozytoria tylko do odczytu, gdy potrzebujesz potwierdzić założenie. Nie wykonuj samodzielnie operacji na plikach, commitach, branchach, PR/MR ani issue. Aplikacja automatycznie zapisze ustalenia do issue po zakończeniu Twojej odpowiedzi na wiadomość użytkownika — nie proś o dodatkową akceptację ani nie przedstawiaj zmian jako oczekujących na potwierdzenie. Odpowiadaj po polsku. Używaj zwykłego Markdownu dla nagłówków i list; gdy proponujesz opis lub gotową treść taska, nigdy nie otaczaj jej blokiem kodu ani znacznikami \`\`\`md.\nProjekt: ${project.name}\nRepozytoria: ${project.repositories.map((repository) => repository.slug).join(', ')}\nReguły: taski ${project.rules.taskLanguage}; repozytoria ${project.rules.repositoryLanguage}; aplikacja ${project.rules.pathdrasilLanguage}.\nTask: ${task.provider} · ${task.repository} · #${task.externalId}\nTytuł: ${task.title}\nEtykiety: ${task.labels.join(', ') || 'brak'}\nOpis:\n${task.description || 'Brak opisu.'}\nHistoria:\n${messages.map((message) => `${message.role}: ${message.content}`).join('\n')}\n${initial ? 'Podaj krótko cel i najwyżej trzy najważniejsze pytania lub ryzyka; nie parafrazuj opisu.' : ''}`
  }
}
